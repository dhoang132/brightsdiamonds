				...
				<div class="article">
					...
					<table width="600px">
						...
						<c:forEach var="row" items="${paymentList}">
							<tr class="order">
								<td><c:set value="${pageContext.request.contextPath}"
										var="contextPath">
									</c:set> <a href="${contextPath}/invoice?id=${row.cartId}">${row.cartId}</a>
								</td>
								<td><c:out value="${row.date}" /></td>
								<td><c:out value="${row.cartTotal}" /></td>
								<td><c:out value="${row.shippingStatus}" /></td>
							</tr>
						</c:forEach>
